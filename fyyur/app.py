...
def venues():
  ...
  places = []
  try:
    for venue in db.session.query.distinct(Venue.city).all():
      if venue.city not in places:
        venue_places = db.session.query.filter_by(city = venue.city).all()
        venues = []
        places_dict = {
          'city': venue.city,
          'state': venue.state,
    ...
    db.session.rollback()
    flash("There might not be any venues in the database yet, hang on!")
    return render_template("pages/home.html")
